...
// PI0 shared code (Q4_0 variant): standard Q4_0 KV cache in HBM, like Q8_0.
//   - Prefix K/V tensors live in the backend buffer as GGML_TYPE_Q4_0
//     (3.83 MiB per inference, vs 27 MiB for fp32 — 7.1x smaller, identical
//     bpw to Q3J1_TQ since both are 18 B / 32 elems).
//   - Quantization (F32 → Q4_0) happens as a ggml_cpy node at the end
//     of the prefix-pass graph; downloaded as Q4_0 bytes.
//   - Concatenation with the current diffusion step's K/V is done by
//     ggml_cpy-into-views over a pre-allocated Q4_0 buffer (ggml_concat
...
//   - ggml_flash_attn_ext consumes Q4_0 K/V directly: K dispatches via
//     type_traits_cpu->vec_dot (Q4_0 vec_dot_type=Q8_0, FA quantizes Q on the fly)
//     and V via type_traits->to_float.
...
// gf is needed when prefix_kv is provided as Q4_0 — the cpy nodes that populate
...
static ggml_tensor * build_gemma_layer(
    ...
    // For Q4_0 K/V, ggml_concat is unusable (concat_any iterates per element
    // with type_size, which is the block size for quantized types). Instead we
    // pre-allocate a full [kv_dim, total_kv] Q4_0 buffer and write prefix +
    // current K/V into it via ggml_cpy-into-views. The prefix slice is a same-type
    // byte memcpy; the current slice triggers F32 → Q4_0 via from_float.
    if (prefix_kv) {
        const int prefix_len_l = prefix_kv->k->ne[1];
        const int total_kv_l   = prefix_len_l + seq_len;
        const int kv_dim       = n_kv_head * head_dim;

        if (prefix_kv->k->type == GGML_TYPE_Q4_0) {
            GGML_ASSERT(gf != nullptr && "build_gemma_layer: gf required for Q4_0 prefix");

            ggml_tensor * full_k = ggml_new_tensor_2d(ctx, GGML_TYPE_Q4_0, kv_dim, total_kv_l);
            ggml_tensor * full_v = ggml_new_tensor_2d(ctx, GGML_TYPE_Q4_0, kv_dim, total_kv_l);
            ...
            // F32 → Q4_0 via from_float (4-bit absmax quantization, fp16 scale per block)
...
// Q4_0 byte buffers — sized via ggml_row_size(GGML_TYPE_Q4_0, kv_dim * prefix_len).
...
static bool run_paligemma_prefix_v2(
    ...
    // Quantize F32 K/V → Q4_0 as the final graph step. ggml_cast allocates
    ...
    for (int il = 0; il < PI0_N_LAYER; il++) {
        char name[64];
        ggml_tensor * k_q = ggml_cast(ctx, kvs[il].k, GGML_TYPE_Q4_0);
        ggml_tensor * v_q = ggml_cast(ctx, kvs[il].v, GGML_TYPE_Q4_0);
        snprintf(name, sizeof(name), "prefix_k_q4_0_%d", il); ggml_set_name(k_q, name);
        snprintf(name, sizeof(name), "prefix_v_q4_0_%d", il); ggml_set_name(v_q, name);
        ggml_set_output(k_q);
        ggml_set_output(v_q);
        kv_q[il*2+0] = k_q;
        kv_q[il*2+1] = v_q;
    ...
    // Download Q4_0 bytes from the GPU buffer to host. 7.1x smaller than fp32.
    {
        const int64_t total_elems = (int64_t)kv_dim * prefix_len;
        const size_t  bytes_per_layer = ggml_row_size(GGML_TYPE_Q4_0, total_elems);
        for (int il = 0; il < PI0_N_LAYER; il++) {
            g_prefix_k_q[il].resize(bytes_per_layer);
            g_prefix_v_q[il].resize(bytes_per_layer);
            // kv_q[il*2+0] is the cpy node; its result tensor is the Q4_0 output.
...
static bool run_expert_step(
    ...
    // Prefix KV tensors — Q4_0 in HBM, like Q4_0/Q8_0 KV cache.
    // Sized in element count along axis 0 (kv_dim must be a multiple of QK4_0=32).
    ggml_tensor * prefix_k_tensors[PI0_N_LAYER];
    ggml_tensor * prefix_v_tensors[PI0_N_LAYER];
    for (int il = 0; il < PI0_N_LAYER; il++) {
        char name[64];
        prefix_k_tensors[il] = ggml_new_tensor_2d(ctx, GGML_TYPE_Q4_0, kv_dim, prefix_len);
        snprintf(name, sizeof(name), "prefix_k_%d", il);
        ggml_set_name(prefix_k_tensors[il], name);
        ggml_set_input(prefix_k_tensors[il]);

        prefix_v_tensors[il] = ggml_new_tensor_2d(ctx, GGML_TYPE_Q4_0, kv_dim, prefix_len);
    ...
    // Set prefix KV data — upload Q4_0 bytes (7.1x smaller than fp32)
    for (int il = 0; il < PI0_N_LAYER; il++) {
