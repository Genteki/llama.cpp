...
// PI0 unified bench harness — KV cache type chosen at runtime via kv_type param.
//   - F32: K/V flow as fp32 (no cast); ggml_concat handles prefix+current concat.
//   - Quantized (Q4_0, Q3J1_TQ, ...): F32→kv_type via ggml_cast at end of prefix
//     graph; cpy-into-views over a pre-allocated buffer replaces ggml_concat
//     (which is broken for quantized types — concat_any iterates per-element
//     with type_size, which is the block size in bytes for quants).
//   - ggml_flash_attn_ext consumes K/V directly via type_traits dispatch.
//   - Storage uses byte buffers sized via ggml_row_size(kv_type, elems) for any type.
...
// gf is needed when prefix_kv is a quantized type — the cpy nodes that populate
...
// For F32/F16 prefix_kv, ggml_concat handles concat and gf may be null.
static ggml_tensor * build_gemma_layer(
    ...
    // For quantized K/V, ggml_concat is unusable (concat_any iterates per element
    // with type_size, which is the block size for quantized types). Instead we
    // pre-allocate a full [kv_dim, total_kv] buffer and write prefix + current K/V
    // into it via ggml_cpy-into-views. The prefix slice is a same-type byte memcpy;
    // the current slice triggers F32 → kv_type via from_float.
    if (prefix_kv) {
        const int prefix_len_l = prefix_kv->k->ne[1];
        const int total_kv_l   = prefix_len_l + seq_len;
        const int kv_dim       = n_kv_head * head_dim;

        if (ggml_is_quantized(prefix_kv->k->type)) {
            GGML_ASSERT(gf != nullptr && "build_gemma_layer: gf required for quantized prefix KV");

            const ggml_type kv_type = prefix_kv->k->type;
            ggml_tensor * full_k = ggml_new_tensor_2d(ctx, kv_type, kv_dim, total_kv_l);
            ggml_tensor * full_v = ggml_new_tensor_2d(ctx, kv_type, kv_dim, total_kv_l);
...
// Byte buffers sized via ggml_row_size(kv_type, total_elems) — works for any type
// (F32: sizeof(float)*elems, quantized: block-aligned compressed bytes).
...
inline ggml_type g_kv_type     = GGML_TYPE_F32;
...
static bool run_paligemma_prefix_v2(
    ...
    int prefix_len,
    ggml_type kv_type            // F32 / F16 / Q4_0 / Q3J1_TQ ...
) {
    ...
    // Cast F32 K/V → kv_type as the final graph step (skipped when kv_type == F32).
    // ggml_cast allocates a new tensor of the target type and wires up a CPY op;
    // the resulting tensor is allocated by gallocr and downloadable via tensor_get.
    ggml_tensor * kv_q[PI0_N_LAYER * 2]; // [il*2+0]=k, [il*2+1]=v
    for (int il = 0; il < PI0_N_LAYER; il++) {
        char name[64];
        ggml_tensor * k_q = (kv_type == GGML_TYPE_F32) ? kvs[il].k : ggml_cast(ctx, kvs[il].k, kv_type);
        ggml_tensor * v_q = (kv_type == GGML_TYPE_F32) ? kvs[il].v : ggml_cast(ctx, kvs[il].v, kv_type);
        snprintf(name, sizeof(name), "prefix_k_%d", il); ggml_set_name(k_q, name);
        snprintf(name, sizeof(name), "prefix_v_%d", il); ggml_set_name(v_q, name);
        ggml_set_output(k_q);
        ggml_set_output(v_q);
        kv_q[il*2+0] = k_q;
        kv_q[il*2+1] = v_q;
    ...
    // Download K/V bytes (any type) for upload to expert step.
    {
        const int64_t total_elems = (int64_t)kv_dim * prefix_len;
        const size_t  bytes_per_layer = ggml_row_size(kv_type, total_elems);
        for (int il = 0; il < PI0_N_LAYER; il++) {
...
static bool run_expert_step(
    ...
    // Prefix KV tensors — typed via g_kv_type (set by run_paligemma_prefix_v2).
    // For quantized types, kv_dim must be a multiple of the block size (32).
    ggml_tensor * prefix_k_tensors[PI0_N_LAYER];
    ggml_tensor * prefix_v_tensors[PI0_N_LAYER];
    for (int il = 0; il < PI0_N_LAYER; il++) {
        char name[64];
        prefix_k_tensors[il] = ggml_new_tensor_2d(ctx, g_kv_type, kv_dim, prefix_len);
        snprintf(name, sizeof(name), "prefix_k_%d", il);
        ggml_set_name(prefix_k_tensors[il], name);
        ggml_set_input(prefix_k_tensors[il]);

        prefix_v_tensors[il] = ggml_new_tensor_2d(ctx, g_kv_type, kv_dim, prefix_len);
    ...
    // Set prefix KV data — upload bytes (size depends on kv_type)
    for (int il = 0; il < PI0_N_LAYER; il++) {
