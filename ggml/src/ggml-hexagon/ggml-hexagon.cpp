...
static ggml_status ggml_backend_hexagon_graph_compute(ggml_backend_t backend, ggml_cgraph * graph) {
    ...
    // Check for cache hit
    const uint64_t graph_uid = 0; // PI0-fork ggml_cgraph has no uid; HTP graph cache disabled
    bool cache_hit = (graph_uid != 0 && sess->cached_graph.uid == graph_uid);
    if (cache_hit) {
        ...
        if (graph_uid != 0) {
            sess->cached_graph.uid = graph_uid;
